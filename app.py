...
Original file is located at
    https://colab.research.google.com/drive/1JzqAJfoUJFc-4Rz_kEz96s31drurHM-H
"""
import os
os.system("pip install -q googletrans==3.1.0a0 deep-translator gTTS langdetect google-generativeai transformers sentencepiece")
# app.py
...
def translate(text, target_lang='en'):
    if target_lang != 'hi' and target_lang != 'en':
        return text
...
# Sentiment and interest detection
...
- Tackle common market-level questions

Be simple, specific, and supportive.
Give complete answers — explain if needed — like you are mentoring a new agent.

Always keep responses relevant to fintech and sales-related tasks.
"""
...
if user_input:
    # 3. Combine last question + current input for context
    full_input = f"{st.session_state.last_question}\nUser: {user_input}"

    # 4. Get response using combined input
    result = chatbot_response(full_input)

    if "error" in result:
        st.error(result["error"])
    else:
        # 5. Save response in session chat history
        ...
        # 6. Update last_question for future reference
        st.session_state.last_question = user_input

# 7. Show chat history
for entry in reversed(st.session_state.chat_history):
    st.markdown(f"**🧑 You:** {entry['question']}", unsafe_allow_html=True)
    st.markdown(f"**🔤 English:** {entry['english']}", unsafe_allow_html=True)
    st.markdown(f"**🗣 Hindi:** {entry['hindi']}", unsafe_allow_html=True)
    if entry["interests"]:
        st.markdown(f"**🔎 Detected Interests:** {', '.join(entry['interests'])}", unsafe_allow_html=True)
    if entry["audio_path"]:
